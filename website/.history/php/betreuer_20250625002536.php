...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_health') {
        ...
        // Insert warnings (only if they don't already exist for today)
        foreach ($warnings as $warning) {
            // Check if this warning already exists for today
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM warnhinweis WHERE patientid = ? AND wert = ? AND DATE(zeitstempel) = CURRENT_DATE");
            $stmt->execute([$patientId, $warning]);
            $existingWarning = $stmt->fetchColumn();
            
            // Only insert if warning doesn't already exist for today
