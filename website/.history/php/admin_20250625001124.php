...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($page === 'betreuer') {
        ...
        if (empty($name) || empty($username) || empty($password) || empty($email) || empty($telefon) || empty($stationID)) {
            header("Location: admin.php?page=betreuer&error=missing_fields");
            exit();
        }
        ...
        try {
            ...
            header("Location: admin.php?page=betreuer&success=betreuer_created");
            exit();
        } catch (PDOException $e) {
            if (strpos($e->getMessage(), 'benutzer_benutzername_key') !== false) {
                header("Location: admin.php?page=betreuer&error=username_exists");
            } else {
                header("Location: admin.php?page=betreuer&error=db_error");
            }
            exit();
        }
        ...
        if (empty($name) || empty($username) || empty($password) || empty($adresse) || empty($geburtsdatum) || empty($email) || empty($telefon) || empty($pflegeart)) {
            header("Location: admin.php?page=patienten&error=missing_fields");
            exit();
        }
        ...
        } elseif (!$stationID && !$betreuerID) {
            // For häusliche Pflege, find any available betreuer
            $stmt = $pdo->prepare("
                SELECT b.betreuerid, COUNT(p.patientid) as patient_count, b.maxpatienten
                FROM betreuer b 
                LEFT JOIN patient p ON b.betreuerid = p.betreuerid
        ...
        try {
            ...
            header("Location: admin.php?page=patienten&success=patient_created");
            exit();
        } catch (PDOException $e) {
            if (strpos($e->getMessage(), 'benutzer_benutzername_key') !== false) {
                header("Location: admin.php?page=patienten&error=username_exists");
            } else {
                header("Location: admin.php?page=patienten&error=db_error");
            }
            exit();
        }
...
<body>
    ...
    if (isset($_GET['success'])) {
        ...
        switch ($_GET['success']) {
            ...
            case 'station_created':
                echo 'Station wurde erfolgreich erstellt!';
                break;
            default:
    ...
    if (isset($_GET['error'])) {
        ...
        switch ($_GET['error']) {
            case 'missing_fields':
                echo 'Fehler: Bitte füllen Sie alle Pflichtfelder aus!';
                break;
            case 'username_exists':
                echo 'Fehler: Dieser Benutzername ist bereits vergeben!';
                break;
            case 'db_error':
                echo 'Fehler: Ein Datenbankfehler ist aufgetreten. Bitte versuchen Sie es erneut.';
                break;
            default:
