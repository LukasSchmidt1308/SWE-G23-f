...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($page === 'betreuer') {
        ...
        $benutzerID = $mysqli->lastInsertId();
        ...
        $stmt = $mysqli->prepare("INSERT INTO Benutzer (Name, Rolle, Benutzername, Passwort, Kontaktdaten) VALUES (?, 'Patient', ?, ?, ?)");
    ...
    if ($page === 'betreuer') {
        ...
        $stmt = $mysqli->prepare("DELETE FROM Betreuer WHERE BetreuerID = ?");
        $stmt->execute([$id]);
        ...
        $stmt = $mysqli->prepare("DELETE FROM Station WHERE StationID = ?");
        $stmt->execute([$id]);
    }
