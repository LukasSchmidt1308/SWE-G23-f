...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_health') {
        ...
        // Clear existing warnings for this patient before inserting new ones
        $stmt = $pdo->prepare("DELETE FROM warnhinweis WHERE patientid = ? AND parameter = 'Gesundheitscheck'");
        $stmt->execute([$patientId]);
        $deletedRows = $stmt->rowCount();
        
        // Insert new warnings only if they exist
        if (!empty($warnings)) {
